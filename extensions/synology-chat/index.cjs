...
// src/core/extensions/synology-chat/index.ts
var index_exports = {};
__export(index_exports, {
  default: () => index_default
});
module.exports = __toCommonJS(index_exports);
var import_synology_chat4 = require("src/core/source/plugin-sdk/synology-chat");

// src/core/extensions/synology-chat/src/channel.ts
var import_synology_chat3 = require("src/core/source/plugin-sdk/synology-chat");
var import_zod = require("zod");

// src/core/extensions/synology-chat/src/accounts.ts
...
function resolveAccount(cfg, accountId) {
  ...
  const envBotName = process.env.MUSTB_BOT_NAME ?? "Must-b";
  return {
    accountId: id,
...
// src/core/extensions/synology-chat/src/client.ts
...
// src/core/extensions/synology-chat/src/runtime.ts
var import_compat = require("src/core/source/plugin-sdk/compat");
var { setRuntime: setSynologyRuntime, getRuntime: getSynologyRuntime } = (0, import_compat.createPluginRuntimeStore)(
  "Synology Chat runtime not initialized - plugin not registered"
);

// src/core/extensions/synology-chat/src/webhook-handler.ts
var querystring = __toESM(require("node:querystring"), 1);
var import_synology_chat2 = require("src/core/source/plugin-sdk/synology-chat");

// src/core/extensions/synology-chat/src/security.ts
var crypto = __toESM(require("node:crypto"), 1);
var import_synology_chat = require("src/core/source/plugin-sdk/synology-chat");
function validateToken(received, expected) {
  if (!received || !expected) return false;
  const key = "must-b-token-cmp";
...
var RateLimiter = class {
  constructor(limit = 30, windowSeconds = 60, maxTrackedUsers = 5e3) {
    this.limit = limit;
    this.limiter = (0, import_synology_chat.createFixedWindowRateLimiter)({
...
async function readBody(req) {
  try {
    const body = await (0, import_synology_chat2.readRequestBodyWithLimit)(req, {
      maxBytes: 1048576,
      timeoutMs: 3e4
    });
    return { ok: true, body };
  } catch (err) {
    if ((0, import_synology_chat2.isRequestBodyLimitError)(err)) {
      return {
        ok: false,
        statusCode: err.statusCode,
        error: (0, import_synology_chat2.requestBodyErrorToText)(err.code)
      };
    }
    return {
      ok: false,
...
function extractTokenFromHeaders(req) {
  const explicit = headerValue(req.headers["x-synology-token"]) ?? headerValue(req.headers["x-webhook-token"]) ?? headerValue(req.headers["x-must-b-token"]);
...
var SynologyChatConfigSchema = (0, import_synology_chat3.buildChannelConfigSchema)(import_zod.z.object({}).passthrough());
...
function createSynologyChatPlugin() {
  return {
    ...
    meta: {
      ...
      blurb: "Connect your Synology NAS Chat to Must-b",
      order: 90
    },
    capabilities: {
      chatTypes: ["direct"],
    ...
    config: {
      ...
      defaultAccountId: (_cfg) => import_synology_chat3.DEFAULT_ACCOUNT_ID,
      setAccountEnabled: ({ cfg, accountId, enabled }) => {
        const channelConfig = cfg?.channels?.[CHANNEL_ID] ?? {};
        if (accountId === import_synology_chat3.DEFAULT_ACCOUNT_ID) {
          return {
            ...cfg,
            channels: {
              ...cfg.channels,
        ...
        return (0, import_synology_chat3.setAccountEnabledInConfigSection)({
          cfg,
    ...
    pairing: {
      ...
      notifyApproval: async ({ cfg, id }) => {
        ...
        await sendMessage(
          account.incomingUrl,
          "Must-b: your access has been approved.",
          id,
          account.allowInsecureSsl
        );
      }
    ...
    security: {
      resolveDmPolicy: ({
        ...
        const resolvedAccountId = accountId ?? account.accountId ?? import_synology_chat3.DEFAULT_ACCOUNT_ID;
        ...
        return {
          ...
          approveHint: "must-b pairing approve synology-chat <code>",
    ...
    gateway: {
      startAccount: async (ctx) => {
        ...
        const unregister = (0, import_synology_chat3.registerPluginHttpRoute)({
...
var plugin = {
  ...
  configSchema: (0, import_synology_chat4.emptyPluginConfigSchema)(),
  register(api) {
